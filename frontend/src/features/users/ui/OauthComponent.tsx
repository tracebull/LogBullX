...
export function OauthComponent() {
  ...
  const handleGitHubLogin = () => {
    if (!GITHUB_CLIENT_ID) {
      toastMessage.error('GitHub OAuth is not configured');
      return;
    }
  ...
  const handleGoogleLogin = () => {
    if (!GOOGLE_CLIENT_ID) {
      toastMessage.error('Google OAuth is not configured');
      return;
    }
  ...
  return (
    <div className="mt-4">
      <div className="space-y-2">
        {GITHUB_CLIENT_ID && (
          <Button variant="outline" onClick={handleGitHubLogin} className="w-full" size="lg">
